...
void redirect(char* file, int to, int append){
    int fd;
    switch(to){
        case 0: fd = open(file, O_RDONLY); break;
        case 1:
        case 2: if(append)
...
int interpretExtras(char** argv, int from, int to){
    ...
    for(int i=to-1; i>=from; --i){
        switch(argv[i][0]){
            case '<': redirect(argv[i+1], 0, 0);
                      offset = i;
                      break;
            case '>': switch(argv[i][1]){
                          case '\0': redirect(argv[i+1], 1, 0); offset = i;
                                     break;
                          case '>':  redirect(argv[i+1], 1, 1); offset = i;
                                     break;
                      }
                      break;
            case '2': switch(argv[i][1]){
                          case '>': redirect(argv[i+1], 2, argv[i][2] == '>');
                                    offset = i;
                                    break;
                      }
                      break;
...
int runCmd(char *cmd){
    ...
    for(int cursor=0; argv[cursor] && (status == PARSING || status == PIPE); cursor++){
        status = PARSING;
        if(argv[cursor][0] == '|')      status = PIPE;
        else if(argv[cursor][0] == '&') status = BACKGROUND;
        else if(argv[cursor+1] == NULL) status = EOC;

        if(status != PARSING){
            if(status == PIPE) pipe(posPipe);
            if(!(lastPid = fork())){
                argv[interpretExtras(argv,lastPipeIndex, cursor+1)] = NULL;
                //pipes
                dup2(prePipe,0);
                if(lastPipeIndex != 0) close(prePipe);
                if(status == PIPE){
                    dup2(posPipe[1],1);
                    close(posPipe[1]);
                    close(posPipe[0]);
                }
                if(status != EOC) argv[cursor] = NULL;

                execvp(argv[lastPipeIndex],argv+lastPipeIndex);
                perror("Failed to execute command");
                _exit(1);
            }
            if(status == PIPE) close(posPipe[1]);
            if(lastPipeIndex != 0) close(prePipe);
            prePipe = posPipe[0];
